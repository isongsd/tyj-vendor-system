                    ...
                    {dayBookings.length > 0 ? dayBookings.map(b => ( 
                        <div key={b.id} className="p-3 bg-gray-800 rounded-lg"> 
                            ...
                            {currentUser?.id === b.vendorId && (
                                <> 
                                    <button onClick={() => callGeminiAPI(`請為「童顏家」產生一篇熱情有活力的社群媒體宣傳短文，用於宣傳擺攤活動。- 活動日期: ${b.date} - 活動地點: ${b.marketCity} ${b.marketName} - 品牌與產品: 童顏家，專注於最新潮流的女鞋、女裝及時尚配件。- 風格要求: 親切、活潑、吸引人，結尾要包含行動呼籲 (例如：快來找我們尋寶！)。- 重要: 文案中請不要提及任何攤主個人姓名。- 請使用繁體中文，並適度加入生動的表情符號 (emoji)。`, setGeminiModal)} className="mt-2 w-full text-sm bg-purple-500 hover:bg-purple-600 text-white font-semibold py-1 px-2 rounded-md">✨ 產生宣傳文案</button> 
                                    {editingSales === b.id ? <SalesInput booking={b} /> : <button onClick={()=> setEditingSales(b.id)} className="mt-2 w-full text-sm bg-gray-600 hover:bg-gray-700 text-white font-semibold py-1 px-2 rounded-md">回報銷售數量</button> }
                                </>
                            )} 
                        </div> 
