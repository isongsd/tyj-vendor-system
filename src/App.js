                        ...
                        <div key={b.id} className="p-3 bg-gray-800 rounded-lg"> 
                            ...
                            {currentUser?.id === b.vendorId && (
                                <> 
                                    ...
                                    {editingSales === b.id ? <SalesInput booking={b} db={db} appId={appId} setEditingSales={setEditingSales} /> : <button onClick={()=> setEditingSales(b.id)} className="mt-2 w-full text-sm bg-gray-600 hover:bg-gray-700 text-white font-semibold py-1 px-2 rounded-md">回報銷售數量</button> }
                                </>
                            )} 
                        </div> 
